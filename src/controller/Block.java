...
public class Block implements Serializable {
    ...
    public Block(int id, String name, int width, int height, int x, int y) {
        if (id <= 0) {
            System.out.println("Block ID must be positive.");
        }
        if (name == null || name.trim().isEmpty()) {
            System.out.println("Block name cannot be null or empty.");
        }
        if (width <= 0 || height <= 0) {
            System.out.println("Block width and height must be positive.");
        }
        this.id = id;
        this.name = name;
        this.width = width;
    ...
    public boolean equals(Object o) {
        ...
        return id == block.id && width == block.width && height == block.height && x == block.x && y == block.y && Objects.equals(name, block.name);
    }
