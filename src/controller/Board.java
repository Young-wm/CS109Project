...
/*这个类中的几个后面非常有用的方法：
*1.直接去new Board()得到的就是一个初始化的“横刀立马”的图:
*        //3 1 1 5
         //3 1 1 5
         //4 2 2 6
         //4 7 8 6
...
public class Board implements Serializable {
    ...
    public void initialize() {
        ...
        initialBlocks.add(new Block(1, "曹操", 2, 2, 1, 0));//1号对应大boss曹操
        initialBlocks.add(new Block(2, "关羽", 2, 1, 1, 2));//2号对应二弟关羽
        initialBlocks.add(new Block(3, "张飞", 1, 2, 0, 0)); // 3号对应三弟张飞
        initialBlocks.add(new Block(4, "赵云", 1, 2, 3, 0)); // 4号对应赵子龙
        initialBlocks.add(new Block(5, "马超", 1, 2, 0, 2)); // 5号对应马超
        initialBlocks.add(new Block(6, "黄忠", 1, 2, 3, 2)); // 6号对应老将军黄忠
        initialBlocks.add(new Block(7, "兵1", 1, 1, 1, 3));
        initialBlocks.add(new Block(8, "兵2", 1, 1, 2, 3));
        initialBlocks.add(new Block(9, "兵3", 1, 1, 1, 4));
        initialBlocks.add(new Block(10, "兵4", 1, 1, 2, 4)); // 4个小兵按照顺序摆放好
        ...
        for (int y = 0; y < block.getHeight(); y++) {
            for (int j = 0; j < block.getWidth(); j++) {
                ...
                if (isValidCoordinate(boardX, boardY)) {
                    grid[boardY][boardX] = block.getId();
                } else {
                    System.out.printf("The block %s can not be put here.", block.getName());
                }
            }
        }
    }
    ...
    public Map<Integer, Block> getBlocksCopy() {
    ...
    public void moveBlockOnBoard(Block block, int newX, int newY) {
        if (block == null) {
            System.out.println("Block to move cannot be null.");
        }

        //先清除旧位置
        for (int i = 0; i < block.getHeight(); i++) {
            for (int j = 0; j < block.getWidth(); j++) {
                if (isValidCoordinate(block.getX() + j, block.getY() + i)) {
                    grid[block.getY() + i][block.getX() + j] = EMPTY_CELL_ID;
                }
            }
        }
        ...
        for (int i = 0; i < block.getHeight(); i++) {
            for (int j = 0; j < block.getWidth(); j++) {
                if (isValidCoordinate(newX + j, newY + i)) {
                    grid[newY + i][newX + j] = block.getId();
                } else {
    ...
    public boolean equals(Object o) {
        ...
        return width == board.width && height == board.height && Objects.deepEquals(grid, board.grid) && Objects.equals(blocks, board.blocks);
    }
