...
public class GamePanel4 extends JPanel {
    ...
    private Map<Integer, Color> blockColors; // Declaration only, initialized in initializeBlockColors()
    ...
    private void updateCellSizeBasedOnPanel() {
        ...
        // 计算每个方向上的单元格大小
        int cellSizeFromWidth = availableWidth / board4.getWidth();
        int cellSizeFromHeight = availableHeight / board4.getHeight();
        
        // 使用较小的值作为单元格大小，确保完整显示
        cellSize = Math.min(cellSizeFromWidth, cellSizeFromHeight);
        
        // 重新计算偏移量，使棋盘居中
        offsetX = (getWidth() - (cellSize * board4.getWidth())) / 2;
        offsetY = (getHeight() - (cellSize * board4.getHeight())) / 2;
    ...
    private void setPreferredSizeBasedOnBoard() {
            Board4 board4 = gameLogic4.getGameState().getBoard();
            int panelWidth = board4.getWidth() * baseCellSize + 2 * offsetX;
            // 总宽度 = 列数 * 单元格大小 + 2 * 偏移量 (左右)
            int panelHeight = board4.getHeight() * baseCellSize + 2 * offsetY;
    ...
    private void handleMouseClick(int mouseX, int mouseY) {
        ...
        if (gridX >= 0 && gridX < gameLogic4.getGameState().getBoard().getWidth() && gridY >= 0 && gridY < gameLogic4.getGameState().getBoard().getHeight()) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        GameState4 gameState4 = this.gameLogic4.getGameState();
        if (gameState4 == null || gameState4.getBoard() == null) {
        ...
        int boardPixelWidth = board4.getWidth() * this.cellSize;
        int boardPixelHeight = board4.getHeight() * this.cellSize;
        ...
        if (allBlocks != null) {
            for (Block4 block : allBlocks.values()) {
        ...
        Block4 selected = this.gameLogic4.getSelectedBlock();
        if (selected != null) {
