...
public class GamePanel extends JPanel {
    ...
    private void setPreferredSizeBasedOnBoard() {
            ...
            int panelWidth = board.getWidth() * cellSize + 2 * offsetX;
            // 总宽度 = 列数 * 单元格大小 + 2 * 偏移量 (左右)
            int panelHeight = board.getHeight() * cellSize + 2 * offsetY;
    ...
    private void handleMouseClick(int mouseX, int mouseY) {
        ...
        if (gridX >= 0 && gridX < gameLogic.getGameState().getBoard().getWidth() && gridY >= 0 && gridY < gameLogic.getGameState().getBoard().getHeight()) {
