...
public class Block4 implements Serializable {
    ...
    public Block4(int id, String name, int width, int height, int x, int y) {
        if (id <= 0) {
            System.out.println("Block4 ID must be positive.");
        }
        if (name == null || name.trim().isEmpty()) {
            System.out.println("Block4 name cannot be null or empty.");
        }
        if (width <= 0 || height <= 0) {
            System.out.println("Block4 width and height must be positive.");
        }
        this.id = id;
        this.name = name;
        this.width = width;
    ...
    public boolean equals(Object o) {
        if (!(o instanceof Block4 block4)) return false;
        return id == block4.id && width == block4.width && height == block4.height && x == block4.x && y == block4.y && Objects.equals(name, block4.name);
    }
