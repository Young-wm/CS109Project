...
/*这个类中的几个后面非常有用的方法：
*1.直接去new Board3()得到的就是一个初始化的“横刀立马”的图:
*        //3 1 1 5
         //3 1 1 5
         //4 2 2 6
         //4 7 8 6
...
public class Board3 implements Serializable {
    ...
    public void initialize() {
        ...
        //接下来我要将所有block初始化：
        List<Block3> initialBlock3s = new ArrayList<>();
        initialBlock3s.add(new Block3(1, "曹操", 2, 2, 1, 0));//1号对应大boss曹操
        initialBlock3s.add(new Block3(2, "关羽", 2, 1, 1, 2));//2号对应二弟关羽
        initialBlock3s.add(new Block3(3, "张飞", 1, 2, 0, 0)); // 3号对应三弟张飞
        initialBlock3s.add(new Block3(4, "赵云", 1, 2, 3, 0)); // 4号对应赵子龙
        initialBlock3s.add(new Block3(5, "马超", 1, 2, 0, 2)); // 5号对应马超
        initialBlock3s.add(new Block3(6, "黄忠", 1, 2, 3, 2)); // 6号对应老将军黄忠
        initialBlock3s.add(new Block3(7, "兵1", 1, 1, 1, 3));
        initialBlock3s.add(new Block3(8, "兵2", 1, 1, 2, 3));
        initialBlock3s.add(new Block3(9, "兵3", 1, 1, 1, 4));
        initialBlock3s.add(new Block3(10, "兵4", 1, 1, 2, 4)); // 4个小兵按照顺序摆放好

        for (Block3 block3 : initialBlock3s) {
            addBlockToBoard(block3);
        }
    ...
    private void addBlockToBoard(Block3 block3) {
        if (block3 == null) {
            return;
        }
        blocks.put(block3.getId(), block3);
        //在添加到棋盘之前，顺便先让block在创建的Map里面与自己的ID对应起来，现在Map里面的序号和block是对应的
        for (int y = 0; y < block3.getHeight(); y++) {
            for (int j = 0; j < block3.getWidth(); j++) {
                int boardX = block3.getX() + j;
                int boardY = block3.getY() + y;
                if (isValidCoordinate(boardX, boardY)) {
                    grid[boardY][boardX] = block3.getId();
                } else {
                    System.out.printf("The block3 %s can not be put here.", block3.getName());
                }
            }
        }
    }
    ...
    public Block3 getBlockById(int blockId) {
        return blocks.get(blockId);
    }
    //这个方法用来查找哪个ID对应哪个Block

    public Map<Integer, Block3> getBlocksCopy() {
    ...
    public void moveBlockOnBoard(Block3 block3, int newX, int newY) {
        if (block3 == null) {
            System.out.println("Block3 to move cannot be null.");
        }

        //先清除旧位置
        for (int i = 0; i < block3.getHeight(); i++) {
            for (int j = 0; j < block3.getWidth(); j++) {
                if (isValidCoordinate(block3.getX() + j, block3.getY() + i)) {
                    grid[block3.getY() + i][block3.getX() + j] = EMPTY_CELL_ID;
                }
            }
        }
        ...
        for (int i = 0; i < block3.getHeight(); i++) {
            for (int j = 0; j < block3.getWidth(); j++) {
                if (isValidCoordinate(newX + j, newY + i)) {
                    grid[newY + i][newX + j] = block3.getId();
                } else {
    ...
    public boolean equals(Object o) {
        if (!(o instanceof Board3 board3)) return false;
        return width == board3.width && height == board3.height && Objects.deepEquals(grid, board3.grid) && Objects.equals(blocks, board3.blocks);
    }
