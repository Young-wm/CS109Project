...
public class Block3 implements Serializable {
    ...
    public Block3(int id, String name, int width, int height, int x, int y) {
        if (id <= 0) {
            System.out.println("Block3 ID must be positive.");
        }
        if (name == null || name.trim().isEmpty()) {
            System.out.println("Block3 name cannot be null or empty.");
        }
        if (width <= 0 || height <= 0) {
            System.out.println("Block3 width and height must be positive.");
        }
        this.id = id;
        this.name = name;
        this.width = width;
    ...
    public boolean equals(Object o) {
        if (!(o instanceof Block3 block3)) return false;
        return id == block3.id && width == block3.width && height == block3.height && x == block3.x && y == block3.y && Objects.equals(name, block3.name);
    }
